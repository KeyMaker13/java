...
public class CalculatorAppletWithTape extends Applet
implements ActionListener {
    public void init() {
        ...
        this.setLayout(new FlowLayout(FlowLayout.
									  CENTER,4,1));
        tape = new List();
        tape.setBackground(backgroundColor);
        add(tape);
        result = new Label(initialString, Label.RIGHT);
        result.setBackground(new Color(255,255,255));
        add(result);
        makeButtons();
        row1 = makePanel(new FlowLayout(FlowLayout.LEFT,
										4,2),backgroundColor);
        row1.add(c);
        row1.add(eq);
        row1.add(div);
        row1.add(times);
        row2 = makePanel(new FlowLayout(FlowLayout.LEFT,
										4,2),backgroundColor);
        row2.add(d7);
        row2.add(d8);
        row2.add(d9);
        row2.add(minus);
        row3 = makePanel(new FlowLayout(FlowLayout.LEFT,
										4,2),backgroundColor);
        row3.add(d4);
        row3.add(d5);
		row3.add(d6);
		...
		add(row2);
		add(row3);
		p12 = makePanel(new BorderLayout(2,2), 
						backgroundColor);
		p12.add("West",d1);
		p12.add("East",d2);
		p120 = makePanel(new BorderLayout(2,2),
						 backgroundColor);
		p120.add("North",p12);
		p120.add("South",d0);
		p3p = makePanel(new BorderLayout(2,2),
						backgroundColor);
		p3p.add("North",d3);
		p3p.add("South",dp);
		p3peq = makePanel(new BorderLayout(2,2),
						  backgroundColor);
		p3peq.add("West",p3p);
		p3peq.add("East",eq2);
		add(p120);
	...
	public void paint(Graphics g) {
		resize(row1.getWidth(),
			   8 * (d1.getHeight()) + tape.getHeight());
		validate();
	}
	...
	private void clearDisplay() {
		...
		printToTape(initialString+" C");
	}
	private void handleOperator(String s) {
		if (!sawAnOpd)
			return;
		printToTape(getDisplay()+" "+s);
		if (lastOptr.equals("+")) 
			calc.add(getDisplay());
		else if (lastOptr.equals("-"))
			calc.subtract(getDisplay());
		else if (lastOptr.equals("*"))
			calc.multiply(getDisplay());
		else if (lastOptr.equals("/"))
			calc.divide(getDisplay());
		else // this is first operator, or last op was =
			calc.setValue(getDisplay());
		result.setText(calc.getValue()+"");
		if (s.equals("="))
			printToTape(getDisplay()+" T");
		lastOptr = s;
		sawDecimal = false;
		newOpd = true;
	}
	...
	private void printToTape(String s) {
		tape.add(s);
		tape.makeVisible(tape.getItemCount()-1);
	}
	...
	static final String initialString = "0.000000";
